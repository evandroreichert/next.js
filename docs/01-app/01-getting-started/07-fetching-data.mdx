...
To improve the initial load time and user experience, you can use streaming to break up the page's HTML into smaller chunks and progressively send those chunks from the server to the client. See the [Streaming guide](/docs/app/guides/streaming) for a comprehensive overview of how streaming works, including the HTTP contract, infrastructure considerations, and trade-offs.
